...
- **A series is not a title.** Beyond the Spiderwick Chronicles prints its series bigger than "The Nixie's Song", and no catalogue files a book under the series. The reader returns `series` apart from `title`, and a series name alone makes the spine unreadable, with the series as its `fragment`. When a title comes back with its series anyway ("Series: Title"), `titleForms` searches and grades each part as well; a match on a part is at best `close`, since only the whole reading can be `exact`. Records are also compared with their subtitle. A catalogue refusing a search (Google's daily quota is a 429) is logged by host, never by URL, which carries the key.
