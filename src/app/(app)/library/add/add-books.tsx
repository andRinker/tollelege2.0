...
export function AddBooks({ readingLevelSystem, suggestions, pairedPhones, scanCursor, phonePairing = true }: AddBooksProps) {
  ...
  const [incomingShelf, setIncomingShelf] = useState<{ slots: ShelfSlot[]; tally?: ShelfTally; otherShelf?: number; quick?: boolean; at: number } | null>(null);
  /**
  ...
  const onScanEvents = useCallback((events: ScanFeedEvent[]) => {
    ...
    for (const event of events) {
      if (event.kind === "book_added") {
        ...
        setArmedGap(null);
        setGapFills({});
        setIncomingShelf({ slots: event.payload.slots, tally: event.payload.tally, otherShelf: event.payload.otherShelf, quick: event.payload.quick, at: event.seq });
        setShelfOpen(true);
      }
    }
